...
class _GreetingHeader extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final greetLine = name != null ? 'Γεια σου, $name 👋' : 'Γεια σου 👋';
      ...
      child: Column(
        ...
        children: [
          ...
          for (final entry in [
            ...
            _BarRow(
                label: entry.$1, percent: entry.$2, color: entry.$3),
...
class _SetupCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final hasKey = settingsAsync.valueOrNull?.isConfigured ?? false;
    final hasProfile = profileAsync.valueOrNull?.isCompleted ?? false;

    if (hasKey && hasProfile) return const SizedBox.shrink();

    final steps = <(bool, String, String, VoidCallback)>[
      (hasKey, '🔑', 'Ρύθμισε το API key',
          () => context.push(AppRoutes.apiSettings)),
      (hasProfile, '👤', 'Συμπλήρωσε το προφίλ σου',
    ...
    return MbCard(
      child: Column(
        ...
        children: [
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Ολοκλήρωσε τη ρύθμιση για να ξεκινήσεις',
                      style: TextStyle(
          ...
          const SizedBox(height: AppSpacing.md),
          for (final (done, emoji, label, action) in steps)
            Padding(
              padding: const EdgeInsets.only(bottom: 8),
              child: InkWell(
                onTap: done ? null : action,
                borderRadius: BorderRadius.circular(8),
                child: Container(
                  padding: const EdgeInsets.symmetric(
